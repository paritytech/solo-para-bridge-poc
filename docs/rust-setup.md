...
**NOTE**: we use some experimental nightly features in our crates (`#![feature(async_closure)]`),
which requires us to set the default rust toolchain to nightly.
...
Configure the Rust toolchain to default to the latest **nightly** version, add nightly and the nightly wasm target:
